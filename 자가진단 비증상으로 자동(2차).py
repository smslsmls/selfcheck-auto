...
for i in range(N):
    ...
    driver.find_element_by_id('user_name_input').send_keys(name[i])#이름 입력
    driver.find_element_by_id('birthday_input').send_keys(birth[i])#생년월일 입력
    driver.find_element_by_id('btnConfirm').click()
    time.sleep(1)
    driver.find_element_by_xpath("//input[@class='input_text_common']").send_keys(pw[i])#비밀번호 입력
    driver.find_element_by_id('btnConfirm').click()
    time.sleep(1)
